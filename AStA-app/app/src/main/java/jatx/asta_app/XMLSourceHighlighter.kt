...
class XMLSourceHighlighter {
    companion object {
        ...
        private var insideComment: Boolean = false
        ...
        private fun isXMLTag(word: String): Boolean {
            return (word.startsWith("<") || word.endsWith(">")) && !isComment(word)
        }

        private fun isResource(word: String): Boolean {
        ...
        private fun wrapWord(word: String): String {
            ...
            if (isResource(word)) {
                return wrapWordWithColor(word, "#FFFF77")
            }
        ...
        fun wrapLine(line: String, isLast: Boolean): String {
            ...
            while (k < line.length) {
                ...
                } else if (c == '/' && k < line.length - 1 && line[k+1] == '>' && !insideStringConst && !insideComment) {
                    wordBuilder.append("/>")
                    words.add(wordBuilder.toString())
                    wordBuilder = StringBuilder()
                    k++
